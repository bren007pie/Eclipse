...
class UndistortThreaded:
    ...
    def undistort(self):
        while True:
            self.img = vs.read()
            self.undist_frame = cv2.remap(self.img, self.map1, self.map2, interpolation=cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT)
            
    def start(self):
        u = Thread(target = self.undistort,args = ())
        u.daemon = True
        u.start()
        return self

    def read(self):
        #return the most recently undistorted frame
        return self.undist_frame
...
#begin undistorted videostream
ud = UndistortThreaded(map1,map2).start()
...
while True:
    #grab frame
    undist_frame = ud.read()#already an array
