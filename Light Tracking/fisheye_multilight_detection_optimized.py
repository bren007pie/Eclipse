...
#define Threaded Pi Cam/Undistort fisheye classes
...
#set up functions for fisheye un-distortion
def undistort(image):              
    #loop until thread is stopped
    while True:
        #grab array of frame
        img = image
        self.undistorted_img = cv2.remap(img, self.map1, self.map2, interpolation=cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT)
        return undistorted_img

def prepUndistort(frame, balance = 1, dim2=None, dim3=None):
    img = frame
...
#get mapping to undistort continually
img_start = vs.read()
(map1, map2) = prepUndistort(img_start) 


#start the thread
u = Thread(target = undistort, args=(frame))
u.daemon = True
u.start()       
...
fps = FPS().start()
#UndistortThreaded must be started after PiCamThreaded because it relies
#on read() function
undist = UndistortThreaded().start()

#warmup
time.sleep(2.0)

#instantiate array to hold frames
frames = np.zeros(5)
